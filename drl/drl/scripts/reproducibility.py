...
for fw in framework_iterator(frameworks=("torch")):
    ...
    for trial in range(3):
        ...
        config = {
            ...
            # 'exploration_config': {"type": "GaussianNoise", "random_timesteps": 10000,  "stddev": 0.1, "initial_scale": 1.0,  "final_scale": 1.0, "scale_timesteps": 1},
            # 'exploration_config': {'type': 'EpsilonGreedy', 'initial_epsilon': 1.0, 'final_epsilon': 0.02, 'epsilon_timesteps': 10000},
        }
        # set_reproducibillity(config['seed'])
        ...
        print(agent.config['explore'],agent.config['exploration_config'])
        trajectory = list()
        for _ in range(8):
            r = agent.train()
